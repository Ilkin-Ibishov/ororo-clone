        ...
        <motion.div
            ...
            <Link onClick={() => { localStorage.setItem("directedPageID", item.id.toString())}} key={item.id} to={`/${localStorage.getItem('selectedContent')}/${item.id}`}>
                <div onMouseEnter={() => setShowPreview(item.id)} onMouseLeave={() => setShowPreview(0)} className='md:w-52 md:h-80 w-28 h-40'>
                    <div hidden={showPreview === item.id}>{item.poster_path?<img src={`${"https://image.tmdb.org/t/p/w500" + item.poster_path}`} alt="" />:<p className=" md:w-full w-28 py-[65%] px-10 bg-primary">No poster photo from backend</p>}</div>
                    <div hidden={showPreview !== item.id} className={`bg-primary p-3 ${showPreview === item.id && "flex flex-col gap-1 relative w-full md:h-80 h-full rounded-md"}`}>
                        <span className='md:text-lg text-sm font-semibold'>{isShow(item)? item.name: isMovie(item)? item.title: null}</span>
                        <div className='flex flex-row flex-wrap gap-2 font-size-css'>
                            {item.genre_ids.map(genre_id => {
                                ...
                                return genre ? <p className='pr-1 md:text-xs text-xs' key={genre_id}>{genre.name}</p> : ''
                            }).filter(Boolean)}
                        </div>
                        <div className='flex flex-row font-size-css gap-4'>
                            ...
                            <div className=" md:block hidden">Rating: {parseFloat(item.vote_average.toFixed(1))}</div>
                        </div>
                        <p className='description-css md:block hidden'>{item.overview}</p>
                    </div>
                </div>
            </Link>
        </motion.div>
